...
type GopherTerminal struct {
	// user is current authorized user
	User string
}
...
func (gt *GopherTerminal) Execute(cmd string) (resp string, err error) {
	...
	// Execute some asked commands if we known them
	switch cmd {
	case "say_hi":
...
/*
	And now we will create our proxy to deliver user and commands to specific objects
*/
...
func (t *Terminal) Execute(command string) (resp string, err error) {
	...
	// but for example we just creating new instance of terminal, set current user and send user command to execution in terminal
